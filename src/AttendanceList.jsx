...
const AttendanceList = () => {
  ...
  useEffect(() => {
    fetch(
      'https://hkm-youtfrest-backend-razorpay-882278565284.asia-south1.run.app/users/attendance-list'
    )
      .then(res => res.json())
      .then(records => {
        setData(records);
                ...
                <Tr key={candidate._id} _hover={{ bg: 'gray.50' }}>
                  ...
                  <Td>
                    {candidate.slot ? (
                      <Tag
                        colorScheme={
                          candidate.slot.toLowerCase().includes('morning')
                            ? 'teal'
                            : candidate.slot.toLowerCase().includes('evening')
                            ? 'orange'
                            : 'gray'
                        }
                        fontWeight="bold"
